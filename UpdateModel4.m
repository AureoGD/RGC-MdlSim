function [H, Phi_w, G_w, Phi_q, G_q, K_tau, P_tau, K_force, P_force, inv_Jc] = UpdateModel4(q, e)

global M N ts Qq Qw Qdw Kp Kd M_total 

persistent C Cf Ce

if isempty(C)
        Cq =  [zeros(12,4) eye(12) zeros(12,12)];
        Cw =  [zeros(12,16) eye(12)];
...
for i = 1:N
    for j=1:M
        if j <= i
            G_w(i*12-11:i*12, j*12-11:j*12) = Cw*A_ext^(i-j)*B_ext;
            G_q(i*12-11:i*12, j*12-11:j*12) = Cq*A_ext^(i-j)*B_ext;
            if i == j
...
for i=1:N
    Phi_w(i*12-11:i*12,:) = Cw*A_ext^(i);
    Phi_q(i*12-11:i*12,:) = Cq*A_ext^(i);
    P_tau(i*12-11:i*12,:) = L*A_ext^(i-1);
    P_force(i*24-23:i*24,:) = FC_mtx*P_tau(i*12-11:i*12,:);
end

H = G_w'*Qw*G_w+G_q'*Qq*G_q+Qdw; %J4
